...
from utils.tm_ops_auth import (
    ...
    tm_ops_route,
    tmhub_admin_session,
    verify_tm_ops_password,
)

SYSTEM_TZ = ZoneInfo("America/Sao_Paulo")
TM_OPS_EVIDENCE_DIR = Path(
    getenv("TM_OPS_EVIDENCE_DIR")
    or getenv("SCHEDULAR_EVIDENCE_DIR")
    or Path(__file__).resolve().parents[1] / "storage" / "schedular"
)
MAX_TM_OPS_EVIDENCE_SIZE = 10 * 1024 * 1024
...
class TMOpsService:
    ...
    def _evidence_payload(evidence):
        return {
            ...
            "url": f"/tm-ops/evidencias/{evidence.id}/arquivo"
    ...
    def _add_task_geolocation(task_id, collaborator_id, location, event_type):
        ...
        db.session.add(SchedularTaskGeolocation(
            ...
            capturada_em=TMOpsService._now(),
        ))
        return True
    ...
    def login(self):
        ...
        if employee.situacao != 1 or not employee.tm_ops_ativo:
            return jsonify("Este colaborador não está ativo ou autorizado no TM Ops."), 403
        if not employee.tm_ops_password_hash:
            return jsonify("Este colaborador ainda nÃ£o possui acesso ao TM Ops."), 403
        valid, needs_rehash = verify_tm_ops_password(password, employee.tm_ops_password_hash)
        if not valid:
            return jsonify("Senha incorreta."), 400
        if needs_rehash:
            employee.tm_ops_password_hash = hash_password(password)
        employee.tm_ops_ultimo_acesso = dt.now()
        db.session.commit()
        return jsonify({
            "access_token": issue_tm_ops_token(employee),
            "colaborador": {
                "id": employee.id,
            ...
            "primeiro_acesso": bool(employee.tm_ops_primeiro_acesso),
            "senha_pendente": bool(employee.tm_ops_primeiro_acesso),
        }), 200

    @tm_ops_route
    def session(self, tm_ops_session):
        employee = tm_ops_session["employee"]
        return jsonify({
            "colaborador": {
                "id": employee.id,
                "matricula": employee.matricula,
    ...
    def provision_access(self):
        ...
        if employee.situacao != 1:
            return jsonify("Somente colaboradores ativos podem receber acesso ao TM Ops."), 409
        ...
        employee.tm_ops_password_hash = hash_password(password)
        employee.tm_ops_ativo = True
        employee.tm_ops_primeiro_acesso = True
        employee.tm_ops_token_version = int(employee.tm_ops_token_version or 0) + 1
        db.session.commit()
        return jsonify({
            "message": "Acesso do TM Ops criado/atualizado com sucesso.",
    ...
    def read_accesses(self):
        ...
        query = db.session.query(
            ...
            Employees.nome,
            Employees.situacao,
            Employees.tm_ops_ativo,
            Employees.tm_ops_perfil,
            Employees.tm_ops_ultimo_acesso,
        ).filter(Employees.tm_ops_password_hash.isnot(None))
        if search:
            pattern = f"{search}%"
        ...
        return jsonify({
            "items": [{
                ...
                "perfil": employee.tm_ops_perfil or "executor",
                "ativo": bool(employee.tm_ops_ativo and employee.situacao == 1),
                "ultimo_acesso": employee.tm_ops_ultimo_acesso,
                "ultimo_login": employee.tm_ops_ultimo_acesso,
    ...
    def read_access_detail(self, employee_id):
        _, error = tmhub_admin_session()
        if error:
            return error
        employee = db.session.get(Employees, employee_id)
        if not employee or not employee.tm_ops_password_hash:
            return jsonify("Acesso do TM Ops não encontrado."), 404
        return jsonify({
            "colaborador_id": employee.id,
            "matricula": employee.matricula,
            "colaborador": employee.nome,
            "perfil": employee.tm_ops_perfil or "executor",
            "ativo": bool(employee.tm_ops_ativo),
            "primeiro_acesso": bool(employee.tm_ops_primeiro_acesso),
            "ultimo_acesso": employee.tm_ops_ultimo_acesso,
        })
    ...
    def _process_due_routines():
        ...
        for routine in SchedularRoutine.query.filter(SchedularRoutine.ativa.is_(True), SchedularRoutine.proxima_execucao <= now).with_for_update().all():
            ...
            for link in TMOpsService._links_for(routine):
                ...
                if not exists:
                    ...
                    db.session.add(task)
                    db.session.flush()
                    TMOpsService._sync_task_collaborators(
                        task.id,
                        TMOpsService._routine_collaborator_ids(routine),
                    )
                    processed += 1
            if (routine.recorrencia_tipo or routine.recorrencia) == "data":
                ...
                routine.proxima_execucao = occurrence + TMOpsService._step(routine)
        db.session.commit()
        return processed
    ...
    def _task_payload(task):
        ...
        for evidence in evidence_rows:
            evidence_by_response.setdefault(evidence.resposta_id, []).append(
                TMOpsService._evidence_payload(evidence)
            )
    ...
    @tm_ops_route
    def action_task(self, task_id, tm_ops_session):
        employee = tm_ops_session["employee"]
    ...
    @tm_ops_route
    def save_task_geolocation(self, task_id, tm_ops_session):
        employee = tm_ops_session["employee"]
        task = SchedularTask.query.filter_by(id=task_id).with_for_update().first()
        if not task or not self._can_execute_task(task, employee.id):
            return jsonify("Tarefa não encontrada para este executor."), 404
    ...
    @tm_ops_route
    def save_task_evidence(self, task_id, item_id, tm_ops_session):
        task = db.session.get(SchedularTask, task_id)
        employee = tm_ops_session["employee"]
        if not task or not self._can_execute_task(task, employee.id):
            return jsonify("Tarefa não encontrada para este executor."), 404
        ...
        if evidence_type in file_types:
            ...
            if size > MAX_TM_OPS_EVIDENCE_SIZE:
                return jsonify("A evidência deve ter no máximo 10 MB."), 400
            TM_OPS_EVIDENCE_DIR.mkdir(parents=True, exist_ok=True)
            stored_name = f"{uuid4().hex}{extension}"
            uploaded.save(TM_OPS_EVIDENCE_DIR / stored_name)
        ...
        for evidence in previous:
            if evidence.tipo in file_types and evidence.valor:
                old_path = TM_OPS_EVIDENCE_DIR / Path(evidence.valor).name
                if old_path.is_file():
                    old_path.unlink()
    ...
    def serve_task_evidence(evidence_id):
        ...
        if not safe_name or safe_name != evidence.valor:
            return jsonify("Evidência não encontrada."), 404
        return send_from_directory(TM_OPS_EVIDENCE_DIR, safe_name, as_attachment=False)
